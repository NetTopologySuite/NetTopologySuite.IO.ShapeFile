...
namespace NetTopologySuite.IO.Dbf
{
    ...
    public class DbfReader : ManagedDisposable, IEnumerable<IReadOnlyDictionary<string, object>>
    {
        ...
        private readonly BinaryBufferReader Binary = new BinaryBufferReader(Dbf.TableDescriptorSize);
        ...
        private void Initialize(Stream stream, Encoding encoding = null)
        {
            ...
            Binary.LoadFrom(DbfStream, Dbf.TableDescriptorSize);

            var version = Binary.ReadDbfVersion();
            if (version != Dbf.Dbase3Version)
                throw new NotSupportedException("Unsupported dBASE version: " + version);

            LastUpdateDate = Binary.ReadDbfLastUpdateDate(); 
            RecordCount = Binary.ReadDbfRecordCount();
            HeaderSize = Binary.ReadDbfHeaderSize();   // 2 bytes (table descriptor header + field descriptor headers + header terminator char 0x0d)
            RecordSize = Binary.ReadDbfRecordSize();
            Binary.Advance(17);

            Encoding = encoding ?? Binary.ReadDbfEncoding() ?? Encoding.UTF8; // null => Try to read encoding from DBF's reserved bytes
            Binary.Advance(2);
            ...
            Binary.LoadFrom(DbfStream, fieldsHeaderSize);
            Fields = new DbfFieldCollection(fieldCount);
            for (int i = 0; i < fieldCount; i++)
            {
                Fields.Add(Binary.ReadDbaseFieldDescriptor(Encoding));
        ...
        {
            BinaryBuffer.TraceToConsole("=== record " + CurrentIndex + "     Stream.Position: " + DbfStream.Position + "  " + new string('=', 50));
            ...
            Binary.LoadFrom(DbfStream, RecordSize);

            var deletedFlag = Binary.ReadByte();
            deleted = deletedFlag == Dbf.DeletedRecordMark;

            for (int i = 0; i < Fields.Count; i++)
            {
                BinaryBuffer.TraceToConsole(Fields[i].Name + "    BinaryBuffer.Position: " + Binary.Position);
                Fields[i].ReadValue(Binary);
            }
        ...
        internal void GetBinaryDiff(DbfReader other, List<string> differences)
        {
            Binary.GetBinaryDiff("DBF", other.Binary, differences);
        }

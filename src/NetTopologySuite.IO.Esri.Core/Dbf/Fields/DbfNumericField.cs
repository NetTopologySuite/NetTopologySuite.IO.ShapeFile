...
namespace NetTopologySuite.IO.Dbf
{
    ...
    public abstract class DbfNumericField<T> : DbfField where T : struct, IConvertible, IFormattable
    {
        ...
        internal override void WriteValue(BinaryBufferWriter recordData)
        {
            ...
            {
                recordData.WriteString(valueText.PadLeft(this.Length, ' '), Length, Encoding.ASCII); // PadLeft for Values
            }
            else
            {
                if (valueText.Length > this.Length)
                ...
                recordData.WriteString(valueText, Length, Encoding.ASCII); 
            }
        }
    ...
    public class DbfNumericField : DbfNumericField<decimal>
    {
        ...
        {
            if (Precision > 0)
                NumberFormat = "0." + new string('0', Precision);
        }

...
namespace NetTopologySuite.IO.Dbf
{
    ...
    {
        private readonly BinaryBufferWriter Binary = new BinaryBufferWriter(Dbf.TableDescriptorSize);
        ...
        private void WriteHeader(Stream stream)
        {
            ...
            // Table descriptor
            Binary.Clear();
            Binary.WriteDbfVersion(Dbf.Dbase3Version);
            Binary.WriteDbfLastUpdateDate(DateTime.Now); //.WriteBytes(GetLastUpdateDate());

            Binary.WriteDbfRecordCount(1);  // Write dummy recordCount. This will be replaced at the end of writing.
            Binary.WriteDbfHeaderSize(headerSize);
            Binary.WriteDbfRecordSize(RecordSize);
            Binary.WriteNullBytes(17);
            Binary.WriteDbfEncoding(Encoding);
            Binary.WriteNullBytes(2);

            // write field description array
            foreach (var field in Fields)
            {
                Binary.WriteDbaseFieldDescriptor(field, Encoding);
            }

            // Now header BinaryDataWriter should be at last byte position
            Binary.WriteByte(Dbf.HeaderTerminatorMark);

            Binary.CopyTo(DbfStream);
        }
        ...
        {
            Binary.Clear();
            Binary.WriteByte(Dbf.ValidRecordMark);

            for (int i = 0; i < Fields.Count; i++)
            {
                Fields[i].WriteValue(Binary);
            }
            RecordCount++;
            Binary.CopyTo(DbfStream);
        }
        ...
        private void FinalizeWriting()
        {
            ...
            Binary.Clear();
            Binary.WriteDbfRecordCount(RecordCount);
            Binary.CopyTo(DbfStream);
        }

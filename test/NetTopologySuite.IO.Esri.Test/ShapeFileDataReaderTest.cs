...
namespace NetTopologySuite.IO.ShapeFile.Test
{
    ...
    public class ShapeFileDataReaderTest
    {
        ...
        {
            using (var reader = Shapefile.Core.ShapefileReader.Open("crustal_test.shp"))
            {
                int length = reader.Fields.Count;
                while (reader.Read(out _))
                {
                    Debug.WriteLine(reader.Fields[(length - 1)]);
                }
            }
        }
        ...
        public void TestReadingCrustalTestShapeFile()
        {
            ...
            using (var reader = Shapefile.Core.ShapefileReader.Open("crustal_test_bugged.shp"))
            {
                int length = reader.Fields.Count;
                while (reader.Read(out _))
                {
        ...
        public void TestReadingAaaShapeFile()
        {
            ...
            {
                using (var reader = Shapefile.Core.ShapefileReader.Open("aaa"))
                {
                    int length = reader.Fields.Count;
                    while (reader.Read(out _))
                    {
        ...
        {
            using (var reader = ShapefileReader.Open("AllNulls"))
            {
        ...
        public void TestReadingShapeFileZ()
        {
            ...
            using (var reader = ShapefileReader.Open("with_M"))
            { // ""
                int index = 0;

                reader.Read(out var geom, out var attributes, out _);
                double firstM = geom.GetOrdinates(Ordinate.M).First();
                Assert.AreEqual(400, firstM);

                while (reader.Read(out geom, out attributes, out _))
                {
        ...
        public void TestReadingShapeFileAfvalbakken()
        {
            ...
            const int distance = 500;
            using (var reader = ShapefileReader.Open("afvalbakken.shp"))
            {
                int index = 0;
                foreach (var feature in reader)
                {
                    Assert.IsNotNull(feature.Geometry);
                    Assert.IsTrue(feature.Geometry.IsValid);
                    Debug.WriteLine(string.Format("Geom {0}: {1}", index++, feature.Geometry));
            ...
            // ShapefileGeometryWriter.WriteGeometryCollection(@"test_buffer", multiBuffer);
        }
        ...
        public void TestSeptPolygones()
        {
            ...
            using (var reader = ShapefileReader.Open("Sept_polygones"))
            {
                int index = 0;
                foreach (var feature in reader)
                {
        ...
        {
            int i = 0;
            foreach (var geom in ShapefileReader.ReadAllGeometries("Issue167.shp"))
            {
        ...
        {
            /*
             * In this case SHP file is broken. It's records have invalid Content Length value. 
             * SHX file has correct Content Legnth value, so it could be used instead.
             * But what to do with opposite case when SHX file is broken and SHP is not?
             * 
             * Bearing in mind that nowadays SHX is useless this issue won't be fixed.
             
            int i = 0;
            foreach (var geom in ShapefileReader.ReadAllGeometries("Volume2.shp"))
            {
                Assert.That(geom, Is.Not.Null, "geom null");
                Console.WriteLine("geom {0}: {1}", ++i, geom);
            }
        ...
        public void EnsureSpecifiedEncodingIsUsed()
        {
            ...
            using (var reader = Shapefile.Core.ShapefileReader.Open("chinese_encoding.shp", encoding))
            {
                Assert.AreEqual(encoding, reader.Encoding);
                Trace.WriteLine(string.Join(",", reader.Fields.Select(f => f.Name)));
                int length = reader.Fields.Count;
                while (reader.Read(out _))
                {
                    Assert.AreEqual("安徽", reader.Fields[1].Value);
                }
